...
float calFactors[LOADCELL_COUNT] = {
  // 5번(10kg), 7번(20kg), 11번(10kg)은 나머지 5kg 로드셀보다 감도가 낮다.
  // 소프트웨어 배율로만 땜질하면 원시 노이즈까지 그만큼 증폭되므로 여기서 직접 맞춘다.
  //
  // 3번은 용량 스케일링(20kg 가정, 107.4)으로는 470g 분동이 8341g로 찍혀
  // 17.75배 어긋났다. 실측 비율로 역산한 값(107.4 * 8341 / 470)으로 교체.
  // 다른 20kg 채널(7번, 104.5)과 동일 계열 수치로는 안 맞았던 걸로 봐서,
  // 3번 소켓의 로드셀이 실제로는 20kg이 아닐 가능성이 있다 — 현장에서 확인 필요.
  424.0, 425.5, 1906.0, 424.2, 223.9, 422.9,
  104.5, 425.0, 423.5, 424.8, 226.4, 424.0
};
